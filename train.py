...
def train(opt):
    ...
    training_set = VOCDataset(opt.data_path, opt.train_set, opt.image_size)
    training_generator = DataLoader(training_set, **training_params)

    test_set = VOCDataset(opt.data_path, opt.test_set, opt.image_size, is_training=False)
    test_generator = DataLoader(test_set, **test_params)

    if torch.cuda.is_available():
        if opt.pre_trained_model_type == "model":
            model = torch.load(opt.pre_trained_model_path)
        elif opt.pre_trained_model_type == "model0":
    ...
    log_path = os.path.join(opt.log_path, "sharp_object_dataset")
    ...
    for epoch in range(opt.num_epoches):
        ...
        for iter, batch in enumerate(training_generator):
            ...
            loss.backward()
            optimizer.step()
            print("Train Batch Epoch: {}/{}, Iteration: {}/{}, Lr: {}, Loss:{:.2f} (Coord:{:.2f} Conf:{:.2f} Cls:{:.2f})".format(
                epoch + 1,
                opt.num_epoches,
                iter + 1,
                num_iter_per_epoch,
                ...
                loss_conf,
                loss_cls))
            writer.add_scalar('Train_Batch/Total_loss', loss, epoch * num_iter_per_epoch + iter)
            writer.add_scalar('Train_Batch/Coordination_loss', loss_coord, epoch * num_iter_per_epoch + iter)
            writer.add_scalar('Train_Batch/Confidence_loss', loss_conf, epoch * num_iter_per_epoch + iter)
            writer.add_scalar('Train_Batch/Class_loss', loss_cls, epoch * num_iter_per_epoch + iter)

            if num_iter_per_epoch == (iter+1):
                writer.add_scalar('Train/Total_loss', loss, epoch)
                writer.add_scalar('Train/Coordination_loss', loss_coord, epoch)
                writer.add_scalar('Train/Confidence_loss', loss_conf, epoch)
                writer.add_scalar('Train/Class_loss', loss_cls, epoch)
        ...
        if epoch % opt.test_interval == 0:
            ...
            loss_conf_ls = []
            loss_cls_ls = []
            num_iter_per_epoch_test = len(test_generator)
            for te_iter, te_batch in enumerate(test_generator):
                ...
                print("Test Batch Epoch: {}/{}, Iteration: {}/{}, Lr: {}, Loss:{:.2f} (Coord:{:.2f} Conf:{:.2f} Cls:{:.2f})".format(
                        epoch + 1,
                        opt.num_epoches,
                        te_iter + 1,
                        num_iter_per_epoch_test,
                        optimizer.param_groups[0]['lr'],
                        batch_loss,
                        batch_loss_coord,
                        batch_loss_conf,
