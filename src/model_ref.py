...
class Yolo2(nn.Module):
    def __init__(self, num_classes,
        ...
        self.stage1_conv8 = nn.Sequential(nn.Conv2d(128, 256, 3, 1, 1, bias=False), nn.BatchNorm2d(256),
                                          nn.LeakyReLU(0.1, inplace=True))
        self.stage1_a_maxpl = nn.MaxPool2d(2, 2)
        self.stage1_conv9 = nn.Sequential(nn.Conv2d(256, 512, 3, 1, 1, bias=False), nn.BatchNorm2d(512),
                                          nn.LeakyReLU(0.1, inplace=True))
        self.stage1_conv10 = nn.Sequential(nn.Conv2d(512, 256, 1, 1, 0, bias=False), nn.BatchNorm2d(256),
                                           nn.LeakyReLU(0.1, inplace=True))
        ...
        self.stage2_b_conv = nn.Sequential(nn.Conv2d(512, 64, 1, 1, 0, bias=False), nn.BatchNorm2d(64),
                                           nn.LeakyReLU(0.1, inplace=True))
        self.stage1_b_conv = nn.Sequential(nn.Conv2d(256, 32, 1, 1, 0, bias=False), nn.BatchNorm2d(32),
                                           nn.LeakyReLU(0.1, inplace=True))

        self.stage3_conv1 = nn.Sequential(nn.Conv2d(256 + 512 + 1024, 1024, 3, 1, 1, bias=False), nn.BatchNorm2d(1024),
                                          nn.LeakyReLU(0.1, inplace=True))
        self.stage3_conv2 = nn.Conv2d(1024, len(self.anchors) * (5 + num_classes), 1, 1, 0, bias=False)
    ...
    def forward(self, input):
        ...
        output = self.stage1_conv8(output)
        residual2 = output
        output = self.stage1_a_maxpl(output)
        output = self.stage1_conv9(output)
        output = self.stage1_conv10(output)
        output = self.stage1_conv11(output)
        output = self.stage1_conv12(output)
        ...
        output_3 = self.stage1_b_conv(residual2)
        batch_size, num_channel, height, width = output_3.data.size()
        output_3 = output_3.view(batch_size, int(num_channel / 16), height, 4, width, 4).contiguous()
        output_3 = output_3.permute(0, 3, 5, 1, 2, 4).contiguous()
        output_3 = output_3.view(batch_size, -1, int(height / 4), int(width / 4))
...
class Yolo3(nn.Module):
    def __init__(self, num_classes,
        ...
        self.stage2_b_conv = nn.Sequential(nn.Conv2d(512, 64, 1, 1, 0, bias=False), nn.BatchNorm2d(64),
                                           nn.LeakyReLU(0.1, inplace=True))
        self.stage1_b_conv = nn.Sequential(nn.Conv2d(512, 64, 1, 1, 0, bias=False), nn.BatchNorm2d(64),
                                           nn.LeakyReLU(0.1, inplace=True))

        self.stage3_conv1 = nn.Sequential(nn.Conv2d(256 + 256 + 1024, 1024, 3, 1, 1, bias=False), nn.BatchNorm2d(1024),
                                          nn.LeakyReLU(0.1, inplace=True))
        self.stage3_conv2 = nn.Conv2d(1024, len(self.anchors) * (5 + num_classes), 1, 1, 0, bias=False)
    ...
    def forward(self, input):
        ...
        output_3 = self.stage1_b_conv(residual2)
        batch_size, num_channel, height, width = output_3.data.size()
        output_3 = output_3.view(batch_size, int(num_channel / 4), height, 2, width, 2).contiguous()
        output_3 = output_3.permute(0, 3, 5, 1, 2, 4).contiguous()
        output_3 = output_3.view(batch_size, -1, int(height / 2), int(width / 2))
